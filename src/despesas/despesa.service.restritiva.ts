...
@Injectable()
export class DespesaServiceRestritiva implements IDespesaService {
  constructor(
  ...
  private validarRegras(
    ...
    if (
      ...
      despesa.descricao.trim().length < 10
    ) {
      throw new RegraNegocioException(
        'Despesa de divulgação deve ter descrição com pelo menos 10 caracteres',
      );
    }
    ...
    if (parlamentar.numeroVotos < 5000 && despesa.valor > 10000) {
      throw new RegraNegocioException(
        'Parlamentares com menos de 5.000 votos não podem lançar despesas acima de R$ 10.000,00',
      );
    }
  }
}